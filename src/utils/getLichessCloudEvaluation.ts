...
const getLichessCloudEvaluation = async (fen: string, token: string, retries = 3) => {
  try {
    ...
    if (retries > 0) {
      ...
      return getLichessCloudEvaluation(fen, token, retries - 1);
    } else {
      throw error;
    }
  }
...
export default getLichessCloudEvaluation;