            ...
            <div className='relative'>
              <input
                ...
                onKeyDown={(e) => {
                  if (e.code === 'ArrowUp') {
                    e.preventDefault()
                    setKeyboardHoverOption((prev) => (prev <= 0 ? autoCompletedUsers.slice(0, 5).length - 1 : prev - 1))
                  } else if (e.code === 'ArrowDown') {
                    e.preventDefault()
                    setKeyboardHoverOption((prev) => (prev >= autoCompletedUsers.slice(0, 5).length - 1 ? 0 : prev + 1))
