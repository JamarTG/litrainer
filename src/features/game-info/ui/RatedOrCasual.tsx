...
const RatedOrCasual: React.FC<RatedOrCasualProps> = ({ isRatedGame }) => {
  ...
  const baseClasses = "px-2 py-0.5 rounded-full text-3xl font-extrabold uppercase tracking-wide";
  ...
  return (
    <span className={`text-4xl ${baseClasses} ${ratedClasses}`} aria-label={`${label} Game`} title={`${label} Game`}>
      {label}
    </span>
  );
};
